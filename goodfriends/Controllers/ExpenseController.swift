...
class ExpenseController {
    ...
    func deleteExpenseAttachment(expenseId: String, completion: @escaping (Result<Void, Error>) -> Void) {
        guard let token = UserDefaults.standard.string(forKey: "userToken"),
              let url = URL(string: "\(API.baseURL)/v1/expenses/\(expenseId)/attachment") else {
            completion(.failure(NSError(domain: "", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid URL or token"])))
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if [200, 204].contains(httpResponse.statusCode) {
                ...
                completion(.failure(NSError(domain: "", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: "Failed to delete expense attachment"])))
            }
        }.resume()
    }
}
